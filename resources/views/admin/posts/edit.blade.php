                    ...
                    <form action="{{ route('posts.update', ['id' => $post->id]) }}" method="post" class="form-horizontal"
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-10">
                                <select name="type" data-placeholder="Choose a Type..." class="chosen-select"
                                    ...
                                    <option value="bachelore" {{ (old("type", $post->type) == 'bachelore' ? "selected":"") }}>Bachelore
                                    </option>
                                    <option value="master" {{ (old("type", $post->type) == 'master' ? "selected":"") }}>Master</option>
                                    <option value="specialization" {{ (old("type", $post->type) == 'specialization' ? "selected":"") }}>Specialization</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="form-group"><label class="col-sm-2 control-label">Note Title:</label>
                            ...
                            <div class="col-sm-10"><input type="text" class="form-control" name="note_title"
                                                          placeholder="Note title" value="{{ old('note_title', $post->note_title) }}">
                            </div>
                        </div>
